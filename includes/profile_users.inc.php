...
WHERE NOT EXISTS (
    SELECT 1
    FROM tbl_dislikes d
    WHERE u.users_id = d.dislikes_disliked_user
    OR u.users_id = d.dislikes_current_user
) AND NOT EXISTS (
    SELECT 1
    FROM tbl_unmatch um
    WHERE u.users_id = um.unmatch_user_id_target 
    OR u.users_id = um.unmatch_user_id_unmatcher
) LIMIT $start, $limit";
$stmt = $db->prepare($sql);
$stmt->execute();
